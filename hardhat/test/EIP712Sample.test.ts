...
describe("EIP712Contract", function () {
  ...
  it("should verify a valid signed message", async function () {
    ...
    await expect(tx).to.emit(contract, "MessageVerified")
      .withArgs(await owner.getAddress(), message.amount, message.message, message.nonce);
  });
  ...
  it("should verify a valid transaction", async function () {
    ...
    const signature = await owner.signTypedData(domain, transactionTypes, transaction);
    ...
    await expect(tx).to.emit(contract, "TransactionVerified")
      .withArgs(transaction.from, transaction.to, transaction.value, transaction.nonce);
  });
  ...
  it("should verify valid user info", async function () {
    ...
    const signature = await owner.signTypedData(domain, userInfoTypes, userInfo);
    ...
    await expect(tx).to.emit(contract, "UserInfoVerified")
      .withArgs(userInfo.wallet, userInfo.username, userInfo.isVerified, userInfo.nonce);
  });
  ...
  it("should fail with an invalid signature", async function () {
    ...
    const fakeSignature = await otherAccount.signTypedData(domain, messageTypes, message);
    await expect(
      contract.verify(
        message.sender,
        message.amount,
